...
output=$(/usr/local/bin/pxctl volume list --all -j | /usr/bin/jq -r '.[] | "created=\"\(.ctime)\",ha_level=\"\(.spec.ha_level)\",format=\"\(.format)\",state=\"\(.state)\",block_size=\"\(.spec.block_size)\",cos=\"\(.spec.cos)\",size=\"\(.spec.size)\",usage=\"\(.usage)\",volume_name=\"\(.locator.name)\""')
...
if [ "$?" == "0" ]; then
  ...
  while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    #created="2019-03-12T06:07:19Z",ha_level="2",format="ext4",state="attached",block_size="4096",cos="low",size="21474836480",usage="474279936",volume_name="swarmstack_grafana"

    volumefill=$(/usr/local/bin/pxctl volume inspect -j `echo "${line}" | cut -f 18 -d '"'` | /usr/bin/jq '.[].usage' | cut -f 2 -d '"')
