...
Here's one example of using cron entries on each node to forward dockerd/portworx/etcd metrics into the Pushgateway so that Prometheus can scrape them. This isn't actually needed as Prometheus will talk directly to the hosts in your swarm, this is only being provided as working examples with real metrics and one way to retrieve your own ephemeral/batch metrics and push them into Prometheus via the Pushgateway:
